...
- 두 줄 스킬명은 라벨의 위·아래 줄을 합칩니다. 첫 줄이 누락되거나 한 글자가 틀려도 가까운 스킬 계열이 하나뿐이면 자동 보정합니다. 레벨은 각 스킬 아이콘의 예상 배지 위치에서 가장 가까운 숫자를 선택하며, 작은 숫자에 한해 신뢰도 0.6까지 허용합니다.
...
- 스킬 레벨은 5~10만 유효한 후보로 인식합니다. 미확정 항목이 있으면 추천/순위를 보류합니다.
