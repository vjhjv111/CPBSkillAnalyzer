...
- 스킬 3칸을 4배 확대한 선수별 이미지를 만들고, 3명씩 한 장의 PNG로 합쳐 요청합니다. 라인업 한 장당 4회 호출합니다. 전체 원본은 전송하지 않습니다.
- OCR boundingPoly 좌표로 이름·포지션·스킬명·레벨을 연결합니다. 두 줄 이름은 위에서 아래로 합칩니다.
- 스킬명 신뢰도가 0.8 미만이거나 계산 조건이 여러 개면 직접 확인합니다. 레벨은 지정된 숫자 영역에 신뢰도 0.8 이상의 숫자가 하나일 때만 확정합니다.
